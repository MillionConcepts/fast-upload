...
def open(name: Path, mode: MCOpenMode) -> tuple[MCFile, str | None]:
    ...
    with close_or_forget(builtin_open(name, mode)) as guard:
        if "r" in mode or "+" in mode:
            magic = os.pread(
                guard.resource.fileno(), LONGEST_COMPRESSION_MAGIC, 0
            )
        else:
            magic = b""
        ...
        if ext is None:
            ...
            raise ValueError(f"MAST does not accept files in {form} format")
...
def check_compression_magic(magic: bytes, name: Path) -> str | None:
    ...
    *canonical* name extension for that type of compression.
    ...
    raise ValueError(
        f"file extension indicates contents {s_are_what} but they {c_are_what}"
    )
