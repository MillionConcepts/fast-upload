...
def st_filetype_with_hduls(
    ...
    hduls = draw(st.lists(st_hdul(ft), min_size=1, max_size=3))
    return ft, hduls
...
def test_random_fits_description_roundtrip(
    ...
    tmp_path = tmp_path_factory.mktemp(
        "random-fits-desc-roundtrip-", numbered=True
    )
    paths = []
    ...
    objects = cast(
        list[DataObject],
        [FITSDataObject.from_text(yaml.dump(o)) for o in objdescs],
    )
    ...
    for p in paths:
        with fits.open(p) as hdul:
            ...
            assert_no_errors(
                errors, "HDUL does not conform to described schema"
            )
