...
def st_parquet_filetype() -> st.SearchStrategy[labels.Filetype]:
    return st_parquet_table_schema().map(
        lambda schema: labels.Filetype(
            standard="parquet",
            filename=[
                labels.FilePattern(
            ...
            objects=[
                make_DataObject(
                    schema=schema, lpath="/filetypes/pqtest/objects/0"
                )
            ],
...
@st.composite
def st_filetype_with_good_table(
    draw: st.DrawFn,
) -> tuple[labels.Filetype, pa.Table]:
...
def test_random_parquet_validation_ok(
    tcase: tuple[labels.Filetype, pa.Table],
) -> None:
    ft, tbl = tcase
    ...
    assert_no_errors(errors, "table does not conform to schema")
