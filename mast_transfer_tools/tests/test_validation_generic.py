...
@st.composite
def st_dtype_name(
    draw: st.DrawFn, choices: Sequence[str] = labels.SUPPORTED_DTYPE_NAMES
) -> str:
    ...
    def restrict_timestamp_regex(rx: str) -> str:
        return (
            "\\A"
            + rx.replace("(?:[1-9][0-9]*)?", "(?:|[2-9]|[1-9]0|[1-9][05]0)")
            + "\\Z"
        )
...
def st_metadata_value() -> st.SearchStrategy[str | bool | float | int]:
    return st.one_of(
        st.text(), st.booleans(), st.floats(allow_nan=False), st.integers()
    )


@st.composite
def st_unequal_metadata_same_type(
    draw: st.DrawFn,
) -> tuple[str | bool | float | int, str | bool | float | int]:
    ...
    if isinstance(v1, float):
        return (
            v1,
            draw(st.floats(allow_nan=False).filter(lambda v2: v2 != v1)),
        )
...
@st.composite
def st_unequal_metadata_diff_types(
    draw: st.DrawFn,
) -> tuple[str | bool | float | int, str | bool | float | int]:
    v1 = draw(st_metadata_value())
    if isinstance(v1, str):
...
def make_DataObject(
    *, schema: list[labels.ColumnObject], lpath: str = "/"
) -> labels.DataObject:
    return labels.DataObject(schema=schema, metadata={}, lpath=lpath)
...
def make_ObjectMetadata(
    ...
    if isinstance(val, re.Pattern):
        ...
        return labels.ObjectMetadata(value=val, value_regex=False, lpath="/")
...
def test_check_column_fail_name_ne(
...
def test_check_schema_ok(record: np.dtype) -> None:
    ...
    columns = [
        make_ColumnObject(name, dtype=ndr(fields[name][0])) for name in names
    ]
...
def test_check_schema_fail_extra_fields(record: np.dtype) -> None:
    ...
    for limit in range(len(names) - 1):
        ...
        errors = v_generic.check_schema(record, dobj)
        assert errors == {
            i: [f"extra field {names[i]!r}"] for i in range(limit, len(names))
        }
...
def test_check_schema_fail_reordered_fields(record: np.dtype) -> None:
    ...
    columns = [
        make_ColumnObject(name, dtype=ndr(fields[name][0])) for name in rnames
    ]
    dobj = make_DataObject(schema=columns)
    errors = v_generic.check_schema(record, dobj)
...
def test_check_meta_fail_ne(
    ...
    assert (
        v_generic.check_meta(vals[0], md)
